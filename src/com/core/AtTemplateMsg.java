...
public class AtTemplateMsg {
	public void atTemplateMsgPage(){
		try{
			WebElement headerElement, activityNameElement, deliveryElement, fileElement, preDefineTempEle, scheduleWebElement, dateElement, hrsElement, minElement,sendElement, delemiterElement, msgCategoryEle, exclutionListEle, throtSizeEle, throtTimeEle,proceedElement;
			String project = AtLaunch.projectName;
			File xmlFile = null, propFile = null;
			if(project.toLowerCase().equals("airtxt")){
				xmlFile = new File("testCases/airtxt/AtTemplateMsg.xml");
			}else if(project.toLowerCase().equals("mreach")){
				xmlFile = new File("testCases/mreach/AtTemplateMsg.xml");
			...
			for(int i = 0; i < list.getLength(); i++){
				...
				templateMsgUrl = element.getAttribute("url");
				...
				templateName = element.getElementsByTagName("pre-defineTemplate").item(0).getTextContent();
			...
			while (entries.hasNext()) {
					...
					if(!templateMsg.equals(null)){
						AtLogin.driver.get(templateMsgUrl);
						...
						System.out.println("Activity Name: TmpMsg-"+activityText+"-"+activityText1+"-"+name);
						System.out.println("File: "+entry.getKey());
						System.out.println("delimiter: "+entry.getValue());
						AtLaunch.bw.write("Activity Name: TmpMsg-"+activityText+"-"+activityText1+"-"+name);
						AtLaunch.bw.newLine();
						AtLaunch.bw.write("File: "+entry.getKey());
						AtLaunch.bw.newLine();
						AtLaunch.bw.write("delimiter: "+entry.getValue());
						AtLaunch.bw.newLine();
						activityNameElement.sendKeys("TmpMsg-"+activityText+"-"+activityText1+"-"+name);
						...
						if(Boolean.valueOf(domestic)){
							...
							if(domesticSenderId.length() > 0){
								...
								AtLaunch.bw.newLine();
								if(project.toLowerCase().equals("airtxt")){
									deliveryElement = AtLogin.driver.findElement(By.xpath("//*[@id='table']/tbody/tr[8]/td[3]/table/tbody/tr[3]/td[1]/span/select"));
						...
						delemiterElement = AtLogin.driver.findElement(By.xpath("//*[@id='table']/tbody/tr[4]/td[3]/table/tbody/tr[6]/td/input"));
						delemiterElement.sendKeys(entry.getValue());
						
						preDefineTempEle = AtLogin.driver.findElement(By.xpath("//*[@id='template']"));
						//preDefineTempEle.click();
						preDefineTempEle.sendKeys(templateName);
			...
			if(!Boolean.valueOf(AtLaunch.groupMsg) && !Boolean.valueOf(AtLaunch.uploadMobile) && !Boolean.valueOf(AtLaunch.uploadMM) && !Boolean.valueOf(AtLaunch.quickMsg) && !Boolean.valueOf(AtLaunch.logout) && !Boolean.valueOf(AtLaunch.navigation)){
				AtLaunch.bw.close();
			}
